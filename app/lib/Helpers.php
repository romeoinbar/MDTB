...
class Helpers
{
	...
	public static function buildEmbed($link, $width=620, $height=360){
		...
		} elseif (substr_count($link, "flashx.tv")) {
			$video_id = explode("/",$link);
			$video_id = $video_id[count($video_id)-2];
			$embed = '<iframe width="'.$width.'" height="'.$height.'" src="http://play.flashx.tv/player/embed.php?hash='.$video_id.'&width='.$width.'&height='.$height.'" frameborder="0" allowfullscreen></iframe>';
